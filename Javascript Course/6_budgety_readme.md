...
- More DOM manipulation and how to remove elements from the DOM
- use the removeChild method to remove the element
- removeChild method require the parent for us to remove the actual element
...
- display month and the year
- Number formatting
- Improve input field UI

#### Updating the percentage in app controller

- Reinforce the concepts and techniques learnt so far
- when should the income percentages be updated, each time new item is added or removed
- these percentages are of each income that the expense represents, hence removeal or addtion will change all the percentages
- similar to update budget create a new function that can be called after ctrlAddItem and ctrlDeleteItem functions
...
#### Updating the percentage in the budget controller
...
var bugetController = (function() {
  ...
  return {
    ...
    calculatePercentages: function() {
      // calculate expense percentage for each expense
      data.allItems.exp.forEach(function(cur) {
        cur.calcPercentage(data.totals.inc);
      });
    },
    getPercenatges: function() {
      var allPercentages;
...
- How to create own forEach function but for the nodeList instead of arrays
...
var UIController = (function() {
  ...
  return {
    ...
    displayPercentages: function(percentages) {
      var fields;
      fields = document.querySelectorAll(DOMstrings.expensesPercentageLabel); // returns a nodeList not an array
      var nodeListForEach = function(list, callback) {
        for (var i = 0; i < list.length; i++) {
          // nodelist still has the length property
          callback(list[i], i);
        }
      };
...
#### Formatting budget Numbers: String Manipiulation

- use string methods to manipulate strings

- all numbers should have a decimal part
- All expenses have - minus sign and space, all incomes should have a + plus sign with a space
- Numbers should be formatted with a comma
- for the above add methods in the UIController
...
var UIController = (function() {
  ...
  return {
    ...
    // since this is internally used, we can make this private for limited access by the UI controller only
    ...
    formatNumber: function(num, type) {
      ...
      dec = numSplit[1];
      if(int.length >3){
        int = int.substr(0,int.length-3)+','int.sunstr(int.length-3,3);
      }
...
#### Display in the current month and year

- how to get the current date using the Date Object Constructor
...
- How and when to use the change event
- change outline color input box based on type of item inc or exp
